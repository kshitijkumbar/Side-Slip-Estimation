...
Q =0.01*diag([10 1 1])*dt^2;  %update this
R = 1*eye(3);     %update this
% npf = 1000; % #particles
%% PF

for p = 200:200:10000
    clear X x_pf sig_pf X pf_weight; 
    npf = p;
...
Xbar = zeros(3,npf,length ( t ));
pf_weight_c = zeros(length ( t ),npf);
z=1;
for i = 2 : length( t )
     i;
...
for j = 1 : npf
    if j<mixture_p*npf
        ...
        y_est( : , j ) = g(X( : , j , i-1 ),delta_rad(i-1),Fxr,Fxf,dt,veh,tire_r,tire_f);
    end
...
beta_mae(z) = norm(beta_estimate - beta_truth)
npf_store(z) = npf
z = z+1;
end
t_PF = toc ;
...
Time_PL = t_PF / length(t);
figure()
plot(npf_store,beta_mae)
%%
